...
class SimpleTable extends Component {
    ...
    render() {
        const {
            data, columns, noMargin, condensed, striped, border, hover, responsive, messageNoRecords
        } = this.props;
        const { key } = this.state;
        const mappedColumns = data && data.length > 0 ? data.map((row, rowIdx) => (
            <tr key={`${key}-${rowIdx}`}>
                {columns.map(col => this.mapCell(row[col.data], col, row, rowIdx))}
            </tr>
        )) : <tr><td colSpan={columns.length} className="text-center">{messageNoRecords?messageNoRecords:'No se encontraron registros'}</td></tr>;
        let headers;
